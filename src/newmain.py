...
class app():
    ...
    def getTexture(self, id):
        ...
        file, _ = QFileDialog.getOpenFileName(self, "Open PNG File", "", "PNG Files (*.png);;All Files (*)", options=options)
    ...
    def generate(self):
        ...
        with open(f'{self.packNamespace}\\function\\tick.mcfunction', 'w') as tick:
            tick.write(self.header + 'execute as @e[type=item_display,tag=' + self.nameSpace + ".custom_block] at @s run function " + self.nameSpace + ":as_blocks")
            tick.close()
        ...
        with open(f'{self.packNamespace}\\function\\check_placed_item_frame.mcfunction', 'a') as file:
            for self.blck in self.blocks.keys():
                ...
                with open(f'{self.packNamespace}\\function\\{self.blck}\\place.mcfunction', 'a') as file2:
                    ...
                    file2.close()
                file.write(self.header + 'execute as @s[tag=' + self.nameSpace + '.' + self.blocks[self.blck]["name"] + '] run function ' + self.nameSpace + ':' + self.blocks[self.blck]["name"] + '/place\n')
        ...
        with open(f'{self.packNamespace}\\function\\as_blocks.mcfunction', 'a') as file:
            for self.blck in self.blocks.keys():
                ...
                file.write(self.header + 'execute as @s[tag=' + self.nameSpace + '.' + self.blocks[self.blck]["name"] + '] run function ' + self.nameSpace + ':' + self.blocks[self.blck]["name"] + '/' + self.blocks[self.blck]["name"] + '\n')
            file.close()
